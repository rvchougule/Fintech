...
import Municipaltaxes_data from "./jsonData/Municipaltaxes_data.json";
...
const Municipaltaxes = () => {
  ...
  const operatorOptions = Municipaltaxes_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Municipaltaxes_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Municipaltaxes_data.providers[index].billerCoverage);
    }
  };
