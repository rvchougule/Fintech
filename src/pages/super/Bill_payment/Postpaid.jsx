...
import PostPaid_data from "./jsonData/PostPaid_data.json";
...
const Postpaid = () => {
  ...
  const operatorOptions = PostPaid_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = PostPaid_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(PostPaid_data.providers[index].billerCoverage);
    }
  };
