...
import Educationfees_data from "./jsonData/Educationfees_data.json";
...
const Educationfees = () => {
  ...
  const operatorOptions = Educationfees_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Educationfees_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Educationfees_data.providers[index].billerCoverage);
    }
  };
