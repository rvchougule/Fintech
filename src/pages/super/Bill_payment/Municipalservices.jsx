...
import Municipalservices_data from "./jsonData/Municipalservices_data.json";
...
const Municipalservices = () => {
  ...
  const operatorOptions = Municipalservices_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Municipalservices_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Municipalservices_data.providers[index].billerCoverage);
    }
  };
  ...
  const formFields = useMemo(() => {
    const staticFields = [
      {
        label: "Municipal services Operator",
        type: "select",
        name: "operator",
