...
import Rental_data from "./jsonData/Rental_data.json";
...
const Rental = () => {
  ...
  const operatorOptions = Rental_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Rental_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Rental_data.providers[index].billerCoverage);
    }
  };
