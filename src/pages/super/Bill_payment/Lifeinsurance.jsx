...
import Lifeinsurance_data from "./jsonData/Lifeinsurance_data.json";
...
const Lifeinsurance = () => {
  ...
  const operatorOptions =  Lifeinsurance_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray =  Lifeinsurance_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage( Lifeinsurance_data.providers[index].billerCoverage);
    }
  };
  ...
  const formFields = useMemo(() => {
    const staticFields = [
      {
        label: "Lifeinsurance tax Operator",
        type: "select",
        name: "operator",
