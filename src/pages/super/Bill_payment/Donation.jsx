...
import Donation_data from "./jsonData/Donation_data.json";
...
const Donation = () => {
  ...
  const operatorOptions = Donation_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Donation_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Donation_data.providers[index].billerCoverage);
    }
  };
