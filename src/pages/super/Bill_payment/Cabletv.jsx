...
import Cabletv_data from "./jsonData/Cabletv_data.json";
...
const Cabletv = () => {
  ...
  const operatorOptions =  Cabletv_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray =  Cabletv_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage( Cabletv_data.providers[index].billerCoverage);
    }
  };
