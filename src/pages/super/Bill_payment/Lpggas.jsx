...
import Lpggas_data from "./jsonData/Lpggas_data.json";
...
const Lpggas = () => {
  ...
  const operatorOptions = Lpggas_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Lpggas_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Lpggas_data.providers[index].billerCoverage);
    }
  };
