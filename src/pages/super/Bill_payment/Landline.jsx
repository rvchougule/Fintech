...
import Landline_data from "./jsonData/Landline_data.json";
...
const Landline = () => {
  ...
  const operatorOptions = Landline_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Landline_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Landline_data.providers[index].billerCoverage);
    }
  };
