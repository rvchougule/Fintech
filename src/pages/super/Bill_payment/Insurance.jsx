...
import Insurance_data from "./jsonData/Insurance_data.json";
...
const Insurance = () => {
  ...
  const operatorOptions = Insurance_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Insurance_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Insurance_data.providers[index].billerCoverage);
    }
  };
