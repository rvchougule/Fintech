...
import Fastag_data from "./jsonData/Fastag_data.json";
...
const Fastag = () => {
  ...
  const operatorOptions = Fastag_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Fastag_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Fastag_data.providers[index].billerCoverage);
    }
  };
