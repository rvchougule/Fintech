...
import Recurringdeposit_data from "./jsonData/Recurringdeposit_data.json";
...
const Recurringdeposit = () => {
  ...
  const operatorOptions =  Recurringdeposit_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray =  Recurringdeposit_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage( Recurringdeposit_data.providers[index].billerCoverage);
    }
  };
  ...
  const formFields = useMemo(() => {
    const staticFields = [
      {
        label: "Recurring Deposit  Operator",
        type: "select",
        name: "operator",
