...
import Loanrepayment_data from "./jsonData/Loanrepayment_data.json";
...
const Loanrepayment = () => {
  ...
  const operatorOptions = Loanrepayment_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Loanrepayment_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Loanrepayment_data.providers[index].billerCoverage);
    }
  };
