...
import Creditcard_data from "./jsonData/Creditcard_data.json";
...
const Creditcard = () => {
  ...
  const operatorOptions =  Creditcard_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray =  Creditcard_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage( Creditcard_data.providers[index].billerCoverage);
    }
  };
  ...
  const formFields = useMemo(() => {
    const staticFields = [
      {
        label: "Creditcard_data  Operator",
        type: "select",
        name: "operator",
