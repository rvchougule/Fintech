...
import Pipedgas_data from "./jsonData/Pipedgas_data.json";
...
const Pipedgas = () => {
  ...
  const operatorOptions = Pipedgas_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Pipedgas_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Pipedgas_data.providers[index].billerCoverage);
    }
  };
