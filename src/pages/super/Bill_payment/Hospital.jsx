...
import Hospital_data from "./jsonData/Hospital_data.json";
...
const Hospital = () => {
  ...
  const operatorOptions = Hospital_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Hospital_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Hospital_data.providers[index].billerCoverage);
    }
  };
