...
import Housingsociety_data from "./jsonData/Housingsociety_data.json";
...
const Housingsociety = () => {
  ...
  const operatorOptions = Housingsociety_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Housingsociety_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Housingsociety_data.providers[index].billerCoverage);
    }
  };
  ...
  const formFields = useMemo(() => {
    const staticFields = [
      {
        label: "Housingsociety tax Operator",
        type: "select",
        name: "operator",
