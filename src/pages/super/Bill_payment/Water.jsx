...
import Water_data from "./jsonData/Water_data.json";
...
const Water = () => {
  ...
  const operatorOptions = Water_data.providers.map((provider) => {
    return `${provider.name} - Coverage: ${provider.billerCoverage}`;
  });

  const customParamResparray = Water_data.providers.map((provider) => {
  ...
  const handleOperatorChange = (selectedValue) => {
    ...
    if (index !== -1) {
      setBillCoverage(Water_data.providers[index].billerCoverage);
    }
  };
